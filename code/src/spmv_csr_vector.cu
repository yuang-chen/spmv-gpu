...
__global__ void spmv_csr_vector_kernel(T * d_val,T * d_vector,int * d_cols,int * d_ptr,int N, T * d_out)
{
    	...
    	// Thread ID in warp
    	int lane = t & (warpSize-1); // lane: 0 - 31

    	// Number of warps per block
    	int warpsPerBlock = blockDim.x / warpSize; // 1024/32=32

    	// One row per warp
    	int row = (blockIdx.x * warpsPerBlock) + (t / warpSize);

    	__shared__ volatile T vals[BlockDim];
    //M is row number, N is column number and nz is the number of entry
    	if (row < N)
    	{
		...
		__syncthreads();
        	...
        	// Reduce partial sums
        	if (lane < 16) vals[t] += vals[t + 16]; // 0 - 15  <= 16 - 31
        	if (lane <  8) vals[t] += vals[t + 8];  // 0 - 7 <= 8 - 15
